...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Could check if right level by: if len(self.a_levels) == level:
        # Here we skip it and assume it is right level.

        action_nodes_set = set()

        for action in self.all_actions:
          new_action_node = PgNode_a(action)
          if new_action_node.prenodes.issubset(self.s_levels[level]):
              action_nodes_set.add(new_action_node)
        ...
        self.a_levels.append(action_nodes_set)
    ...
    def add_literal_level(self, level):
        ...
        for action_node in self.a_levels[level - 1]:
            for s_node in action_node.effnodes:
                check_if_already_added = False
                for added_s_nodes in s_nodes_set:
                    if added_s_nodes.__eq__(s_node):
                        added_s_nodes.parents.add(action_node)
                        action_node.children.add(added_s_nodes)
                        check_if_already_added = True
                if not check_if_already_added:
                    s_nodes_set.add(s_node)
                    s_node.parents.add(action_node)
                    s_node.children.add(action_node)

        self.s_levels.append(s_nodes_set)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for add_eff in node_a1.action.effect_add:
            for rem_eff in node_a2.action.effect_rem:
                if add_eff == rem_eff:
                    return True

        for add_eff in node_a2.action.effect_add:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for pos_lit in node_a1.action.precond_pos:
            for rem_eff in node_a2.action.effect_rem:
                if pos_lit == rem_eff:
                    return True

        for pos_lit in node_a2.action.precond_pos:
            for rem_eff in node_a1.action.effect_rem:
                if pos_lit == rem_eff:
                    return True

        for neg_lit in node_a1.action.precond_neg:
            for add_eff in node_a2.action.effect_add:
                if neg_lit == add_eff:
                    return True

        for neg_lit in node_a2.action.precond_neg:
            for add_eff in node_a1.action.effect_add:
                if neg_lit == add_eff:
                    return True
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for a_1 in node_s1.parents:
            any_non_mutex = False
            for a_2 in node_s2.parents:
                if not a_1.is_mutex(a_2):
                    any_non_mutex = True
            if any_non_mutex == False:
                all_mutex = True
        ...
        # Check if any parent is father of both:
        for a_1 in node_s1.parents:
            if a_1 in node_s2.parents:
                all_mutex = False
    ...
    def h_levelsum(self) -> int:
        ...
        This function is called as a method of an instance of the Planning Graph Class (PG),
        which is made under the h_pg_levelsum method of the Problem class. The PG instance is
        formed using a state that is carried within a Node instance, which in turn was passed
        along by the Search.py program, under the best_first_graph_search method. Thus, this
        method calculates an estimation of the distance of each of the goal litereals in a given
        node from the goal node and returning their sum. It is repeatedly called during best first search.
        :return: int
        """
        level_sum = 0
        level_costs = []
        for goal in self.problem.goal:
            ...
            while not found:
                for sn in self.s_levels[level]:
                    if sn.symbol == goal and sn.is_pos:
                        level_costs.append(level)
                        found = True
                level += 1
        level_sum = sum(level_costs)
